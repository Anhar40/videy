...
- **Vercel**: `api/index.js` exports the Express app (`module.exports = app` from `server.js`). `vercel.json` rewrites `/api/(.*)` → `/api/index`; static files (`index.html`, `preview.html`, `videos.json`) are served by Vercel filesystem. `app.listen(PORT)` runs only when `require.main === module` (local dev). `.vercelignore` excludes `.env`, `uploads/`, `videos/`. Set the Cloudinary env vars in the Vercel dashboard (not in git). **Vercel crashes at module load** if `fs.mkdirSync`/`express.static` run (read-only filesystem) or `new URL(process.env.CLOUDINARY_URL)` is called when env is missing — `server.js` guards with `isVercel` (`process.env.VERCEL`) and handles missing/malformed env without crashing.
