...
- **Plain Node server**: runs directly with `node server.js`; `app.listen(PORT)` is unconditional at the end of `server.js`. `api/` and `vercel.json` were **removed** (no serverless/Vercel anymore). Old `isVercel`/`getDiagnostics()`/`findFrontendFile()` vestiges may still exist in `server.js` but are inert (never true outside Vercel); `/api/diagnose` is a harmless debug endpoint. `.env` holds the Cloudinary URL.
